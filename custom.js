...
function loadDefaultMap() {
  ...
  if (!map) {
    map = L.map('map').setView([46.603354, 1.888334], 6); //  les coordonnées et le niveau de zoom souhaités
    ...
    mapContainer.map = map; // référence de la carte dans l'élément HTML pour une utilisation ultérieure

    const defaultGpxUrl = 'gpxpoints/track.gpx'; // l'url du fichier gpx
...
function loadGPX(url, map) {
  ...
  fetch(url)
    ...
    .then(gpxData => {
      ...
      map.gpxLayer = gpxLayer; // on stock la référence de la couche GPX dans la carte pour une utilisation ultérieure
    })
    .catch(error => {
...
fetch('http://localhost:5004/api/itineraires')
  ...
  .then(data => {
    ...
    // ttt des données, parcours du tableau avec la fonction map
    ...
    figures.forEach(figure => {
      figure.addEventListener('click', () => {
        ...
        if (!map) {
          ...
          mapContainer.map = map; // on stock la référence de la carte dans l'élément HTML pour une utilisation ultérieure
        }
